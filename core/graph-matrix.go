	...
	for _, income := range loopNodes {
		...
		if income.isSelfLoop {
			...
			err = g.insertOrSkipNodeOnMatrix(&NodeOutput{
				...
				Anchor: &Anchor{
					Type:        AnchorLoop,
					Orientation: AnchorOrientationBottomRight,
					From:        item.Id,
					To:          id,
				},
				IsAnchor:         true,
		...
		err = g.insertOrSkipNodeOnMatrix(&NodeOutput{
			...
			Anchor: &Anchor{
				Type:        AnchorLoop,
				Orientation: AnchorOrientationTopRight,
				From:        item.Id,
				To:          id,
			},
			IsAnchor:         true,
		...
		err = g.insertOrSkipNodeOnMatrix(&NodeOutput{
			...
			Anchor: &Anchor{
				Type:        AnchorLoop,
				Orientation: AnchorOrientationTopLeft,
				From:        item.Id,
				To:          id,
			},
			IsAnchor:         true,
...
func (g *GraphMatrix) insertSplitOutcomes(item *NodeOutput, state *state, levelQueue *TraverseQueue) (err error) {
	...
	for _, outcomeId := range outcomes {
		...
		err = g.insertOrSkipNodeOnMatrix(&NodeOutput{
			...
			Anchor: &Anchor{
				Type:        AnchorSplit,
				Orientation: AnchorOrientationBottomLeft,
				From:        item.Id,
				To:          outcomeId,
			},
			IsAnchor:         true,
